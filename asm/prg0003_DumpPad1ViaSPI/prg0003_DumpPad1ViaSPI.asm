; $Id: prg0003_DumpPad1ViaSPI.asm 1590 2024-01-03 14:39:46Z sow $
...
MAIN_LOOP:
    ...
    ora <PAD1_RELEASE
    beq MAIN_LOOP_END
    jsr STORE_PAD1_STRING
    jsr SEND_MESSAGE_VIA_SPI
MAIN_LOOP_END:
    jmp MAIN_LOOP

; FUNCTION: Store PAD1 status and 'CR' with the terminate charactor '$00'
; INPUT: PAD1_CURRENT
; OUTPUT: PAD1_STRING[0]-PAD1_STRING[9] (must to be placed in zero page)
; BROKEN: P A X Y
; STACK: 1
; CONSTRAINT: PAD1_STRING needs 10 byte. (eight chars, CR and NULL)
STORE_PAD1_STRING:
    ldx #$00
STORE_PAD1_STRING_CR:
    lda #$0D
    sta <PAD1_STRING,x
    inx
    ldy #$08
    lda <PAD1_CURRENT
    pha
STORE_PAD1_STRING_LOOP:
    and #$01
    ora #$30
    sta <PAD1_STRING,x
    inx
    dey
    beq STORE_PAD1_STRING_TERMINATE
    pla
    lsr a
    pha
    bpl STORE_PAD1_STRING_LOOP
STORE_PAD1_STRING_TERMINATE:
    pla
    lda #$00
    sta <PAD1_STRING,x
STORE_PAD1_STRING_END:
	rts

; FUNCTION: Send string
; INPUT: PAD1_STRING[0]-PAD1_STRING[9] (must to be placed in zero page)
; OUTPUT: SPI
; BROKEN: P A X Y
; STACK: 3 (SPI_SEND_A_BYTE consume 3 byte)
; CONSTRAINT: SPI_SEND_A_BYTE is necessary
...
MESSAGE:
    db "Send PAD1 status to HOST via SPI", $00
